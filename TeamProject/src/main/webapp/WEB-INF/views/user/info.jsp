...
<body>
							...
							<div class="panel panel-success">
								...
								<table class="table table-hover">
									...
									<c:forEach items="${info.rvoList }" var="ren">
										<tr id="ren${ren.num}">
											<td><a
												href="<c:url value="/book/read?bnum=${ren.booknum}"/>"
												title="${ren.bookname}">${ren.bookname}</a></td>
											<td>${ren.rendate }</td>
											<td>${ren.returndate }</td>
											<td>${ren.day }</td>
											<c:if test="${ren.status==1 }">
												<td><a
													href="javascript:returnBook(${ren.num },${ren.booknum });">반납하기</a></td>
											</c:if>
							...
							<div class="panel panel-success">
								...
								<table class="table table-hover">
									...
									<c:forEach items="${info.bvoList }" var="boo">
										<tr id="boo${boo.num}">
											<td><a
												href="<c:url value="/book/read?bnum=${boo.booknum }"/>"
												title="${boo.bookname}">${boo.bookname}</a></td>
											<c:choose>
												<c:when test="${!boo.ok }">
													<td>${boo.rendate }</td>
												...
												<c:otherwise>
													...
													<td><a
														href="javascript:rental(${boo.booknum },${boo.num});">대여</a>
														/ <a href="javascript:cancel(${boo.num });">취소</a></td>
												</c:otherwise>
											</c:choose>
										</tr>
									</c:forEach>
								...
								<table class="table table-hover">
									...
									<c:forEach items="${info.rvoList }" var="ren">
										<tr id="ren${ren.num}">
											...
											<td><a
												href="javascript:returnConfirm(${ren.num },'${ren.rentalUser}');">반납
													승인</a></td>
										</tr>
									</c:forEach>
								</table>
						...
						<div class="panel panel-default">
							<div class="panel-body">
								<ul class="list-inline" style="text-align: center;">
									<li><a class="link" href="<c:url value="/"/>">&bull;
											메인</a></li>
									<li>
									<li></li>
									<li><a class="link"
										href="<c:url value="/user/edit?id=${id}"/>">&bull;정보수정</a></li>
									<li></li>
									<li><a class="link" href="#secessionModal"
										data-toggle="modal">&bull;회원탈퇴</a></li>
								</ul>
							</div>
						</div>
					</div>
	...
	<!-- 탈퇴 모달 -->
	<div class="modal fade" id="secessionModal" tabindex="-1"
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="form-group">탈퇴를 원하시면 비밀번호를 입력해주세요.</div>
	...
	<form id="seceForm">
		<input type="hidden" name="${_csrf.parameterName }"
			...
			value="${info.vo.userid }"> <input type="hidden" name="upwd">
	</form>
	<!--반납  요청 -->
	<form id="returnForm">
		<input type="hidden" name="${_csrf.parameterName }"
			value="${_csrf.token }"> <input type="hidden" name="num">
	...
	<form id="rentalForm">
		<input type="hidden" name="${_csrf.parameterName }"
			value="${_csrf.token }"> <input type="hidden" name="booknum">
		<input type="hidden" name="bookingnum"> <input type="hidden"
