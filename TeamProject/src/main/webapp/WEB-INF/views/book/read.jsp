					...
					<div class="heading">
						<div class="panel panel-default">
							<div class="panel-body">
								<div id="headimg">
									<img class="img-rounded" width="200px" height="210px"
										src="img?coverName=${book.vo.coverName} " />
								</div>
								<div id="headtable">
									<table class="table table-striped">
										...
										<tr class="success">
											<td class="cell author"><a
												href="javascript:clickSearch('author','${book.vo.author }');">${book.vo.author }</a></td>
										</tr>
										<tr class="success">
											<td class="cell publisher"><a
												href="javascript:clickSearch('publisher','${book.vo.publisher }');">${book.vo.publisher }</a></td>
										</tr>
									</table>
									<c:choose>
																...
																<c:when test="${book.bookingnum<5&&ok==false }">
																	<sec:authorize access="hasAuthority('USER')">
																		<a class="btn btn-theme"
																			href="<c:url value="javascript:booking();"/>">예약가능</a>
																	</sec:authorize>
										...
										<c:otherwise>
											<div class="panel panel-warning">
												...
												<div class="panel-body">
													<sec:authorize access="hasAuthority('USER')">
														<a class="btn btn-theme"
															href="<c:url value="javascript:rental();"/>">대여하기</a>
													</sec:authorize>
													<sec:authorize access="!isAuthenticated()">
...
${book.vo.bcontents }
					...
					<div class="panel panel-default">
						<div class="panel-body">
							<ul class="list-inline" style="text-align: center;">
								...
								<li>
								<li></li>
								<li><a class="link" href="javascript:list();">&bull;
										목록보기</a></li>
								<li></li>
								<sec:authorize access="hasAnyAuthority('MANAGER','ADMIN')">
									<li><a class="link"
										href="<c:url value="/book/edit?bnum=${book.vo.bnum}"/>">도서
											수정</a></li>
									<li></li>
									<li><a class="link" href="javascript:deleteBook();">도서
											삭제</a></li>
								</sec:authorize>
							</ul>
						</div>
	...
	<form action="search" method="post" id="list">
		<input type="hidden" name="${_csrf.parameterName }"
			...
			value="${category}"> <input type="hidden" name="keyword"
			value="${keyword}">
	</form>
	<!-- 클릭 서치  -->
	...
	<form id="rentalForm">
		<input type="hidden" name="${_csrf.parameterName }"
			...
			type="hidden" name="booknum" value="${book.vo.bnum}">
	</form>
	<!--부킹 폼  -->
	<form id="bookingForm">
		<input type="hidden" name="${_csrf.parameterName }"
